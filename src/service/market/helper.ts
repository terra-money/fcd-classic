...
export function getTargetDatetime(datetime: Date, interval: string): number {
  const msc = Math.max(MIN_DURATION, parseDuration(interval))
...
export async function getOnedayBefore(): Promise<CoinByDenoms> {
  ...
  // console.log("LOL ONE DAY ", denomPrices)
  // console.log(new Date(getTargetDatetime(new Date(oneDayBefore), '1m')))
